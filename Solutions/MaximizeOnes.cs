...
We are allowed to do exactly one move: We must choose two indices 
i and j (1 <= i <= j <= n) and flip all values ak for which their positions are in 
range [i, j] (that is i <= k <= j). Flip the value of x means to apply operation x = 1 - x.

The goal is - After exactly one move to obtain the maximum number of ones. 

Input:-

The first line of the input contains an integer n (1 <= n <= 10^6).
...
That flip changes the sequence, it becomes: [1 1 1 0 1].
So, it contains four ones. There is no way to make the whole sequence equal to [1 1 1 1 1].

In the second case, flipping only the second and 
the third element (i=2, j=3) will turn all numbers into 1.
